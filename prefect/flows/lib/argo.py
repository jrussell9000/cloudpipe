...
def parse_submission_rate(value: object) -> float:
    """Parse a live pacing value: workflows submitted per minute, `0` to disable.

    Anything else raises, so that `cap_reader` holds the last good rate instead of
    acting on a typo. A negative or non-finite rate has no sensible meaning, and
    quietly reading one as "disabled" would reopen exactly the burst pacing exists
    to prevent.
    """
    rate = float(str(value).strip())
    if not math.isfinite(rate) or rate < 0:
        raise ValueError(f"submission rate must be a finite number >= 0, got {value!r}")
    return rate


def wait_for_pace(
    last_submitted_at: float | None,
    read_rate: Callable[[], float],
    *,
    max_sleep: float,
    sleep: Callable[[float], None] = time.sleep,
    clock: Callable[[], float] = time.monotonic,
) -> float:
    """Block until the next submission keeps within the live rate; return seconds waited.

    The concurrency cap bounds how many workflows stand at once, not how fast they
    arrive. From an empty queue the cap admits its whole width in minutes, and every
    one of those workflows first-reconciles together: on 2026-09-14, 300 workflows in
    7 minutes drove the control plane into `etcdserver: throttle`, which leaked the
    `globus-transfer` semaphore in the controller's memory (a 2-hour transfer stall)
    and stranded four WorkflowTaskResults at `report-outputs-completed=false` (#393).
    Neither recovered without a hand fix. Spacing arrivals is the fix; lowering the
    cap would cut steady-state throughput to buy the same thing.

    Spacing is measured from the previous submission, so pacing costs little once
    the cap is binding: a workflow admitted after a long slot wait is already past
    its interval, and only slots that free up together are spread out. The rate is re-read at least every `max_sleep` seconds, so an
    operator who raises it mid-wait is not left waiting out the old interval.
    """
    ...
    while True:
        ...
        if rate == 0:
            break
        remaining = last_submitted_at + 60.0 / rate - clock()
        if remaining <= 0:
            break
        sleep(min(remaining, max_sleep))
