...
resource "aws_glue_catalog_table" "costs_compacted" {
  ...
  parameters = merge(local.compacted_partition_projection.costs, {
    ...
    # union_by_name=true.
    #
    # 1.3 adds source + scraped_total_cost_usd (reconstructed rows). A version
    # outside this enum is not a projected partition and its records return
    # nothing at all, so this must be bumped with the dataclass.
    "projection.schema_version.values" = "1.0,1.1,1.2,1.3"
  })
  ...
  storage_descriptor {
    ...
    columns {
      # Schema 1.3+ — "kubecost" | "reconstructed"; NULL on older partitions,
      # which are all scraped. Declared in CostAllocation's field order, like
      # every column here: the Parquet files are written from the dataclass, and
      # a Glue order that disagrees with the file risks index-based reads
      # resolving the wrong column.
      name = "source"
      type = "string"
    }
    columns {
      # Schema 1.3+ — what a reconstructed row replaced; NULL otherwise.
      name = "scraped_total_cost_usd"
      type = "double"
    }
    columns {
      name = "pipeline"
...
resource "aws_glue_catalog_table" "costs" {
  ...
  storage_descriptor {
    ...
    ser_de_info {
      serialization_library = "org.openx.data.jsonserde.JsonSerDe"
      parameters = {
        paths                   = "completed_at,cpu_cost_usd,date,gpu_cost_usd,memory_cost_usd,pipeline,scrape_age_days,schema_version,scraped_total_cost_usd,source,subject,total_adjustment_usd,total_cost_usd,workflow_name"
        "ignore.malformed.json" = "true"
      }
    }
    ...
    columns {
      # Schema 1.3+ — "kubecost" | "reconstructed". NULL on older records, which
      # are all scraped: read as COALESCE(source, 'kubecost').
      name = "source"
      type = "string"
    }
    columns {
      # Schema 1.3+ — the scraped figure a reconstructed row replaced; NULL
      # otherwise, including on every scraped row.
      name = "scraped_total_cost_usd"
      type = "double"
    }
    columns {
