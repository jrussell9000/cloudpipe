...
Constructed as a raw dict in `build_record()`, assembled from two sources: `fsqc-results.csv`
(the core metrics, rotation, and outlier counts) and the per-module `status/{session}/status.txt`
files (the `*_status` codes). Until 2026-09-16 a third source, `outliers/all.regions.stats`,
supplied the hypothalamic volumes; that module is retired (see the hypothalamus rows below).
Matches `FsqcQC` in `src/metrics/schemas.py`.
...
QC images (not part of the record): `fsqc/{subject}/{session}/*.png` — the hippocampus overlays
plus a whole-brain `screenshots-{session}.png` (and, for records before 2026-09-16, a
`hypothalamus.png`). The browsable summary page
...
| `hypothalamus_whole_left_mm3`, `hypothalamus_whole_right_mm3` | float? | **Retired 2026-09-16 — `NULL` on every row written since.** Whole hypothalamus volume per side (mm³) from FreeSurfer's hypothalamic subunits, lifted from `outliers/all.regions.stats`. The pipeline dropped that segmentation for FastSurfer's HypVINN (`derivatives/fastsurfer/{subj}/{ses}/stats/hypothalamus.HypVINN.stats`), which fsqc cannot QC. **Do not use the historical values either**: the segmentation covered only each subject's *last* session (a repeated-`--s` bug), and the `NULL`s once documented here as "common and not an error" for "uncovered sessions" were that bug. Kept as columns so the schema is unchanged |
| `metrics_status`, `outlier_status`, `hippocampus_status`, `hypothalamus_status` | int? | Per-module exit codes from `status/{session}/status.txt`. `0` = the module ran clean, non-zero = it degraded (missing input, `NaN` output), `NULL` = it never reported. **`0` and `NULL` are not the same thing** — that distinction is the only way to tell "ran and found nothing" from "never ran", so read the relevant status alongside any `NULL` metric. `hypothalamus_status` is always `NULL` from 2026-09-16 (module retired, see above) |
