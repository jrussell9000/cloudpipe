...
The BOLD→T1w registration (SynthMorph, ADR 002) produces a transform referenced to the FreeSurfer conformed space: SynthMorph receives FastSurfer's `T1.mgz` (also in conformed space) as the fixed image and writes the LTA transform with the conformed volume as its target coordinate frame. The ITK-format transform used in functional preprocessing (`_desc-bold2t1w_itk.txt`) is therefore a BOLD→conformed transform.
...
- FastSurfer must complete successfully before T1w→MNI registration can run (the `orig.mgz` and `brainmask.mgz` are staged as single objects from the session's FastSurfer derivatives tree, `derivatives/fastsurfer/{subj}/{ses}/mri/` — see [ADR 017](017-exploded-derivatives-over-tarballs.md)); there is no path to run T1w→MNI without FastSurfer
