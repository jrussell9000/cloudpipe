...
- `functional-preprocessing-session-template` can be submitted standalone without running the full pipeline — all its inputs (BOLD, registration outputs, FastSurfer `aseg.auto.mgz` / `surf/` / `_links.json`, MNI template) are declared as S3 artifact inputs
...
`subregion-segmentation`'s segmentation pods now stage the FastSurfer derivatives onto their own private `emptyDir`s (the `hydrate-fastsurfer-template` pod is gone — each segmentation pod declares those inputs directly) and checkpoint each region to its final S3 location immediately after that region completes, rather than relying on a retry-persistent volume. (As originally written this said tarballs — `{subj}_{region}.tar.gz` keys. Since [ADR 017](017-exploded-derivatives-over-tarballs.md) both the staged FastSurfer trees and the region outputs are exploded, one object per file, and a region's checkpoint is `derivatives/subregions/{subj}/{region}/` with `_complete.json` written last. `segment-subregions-dl-template` was later removed in #401, leaving `segment-subregions-gems-template` as the only segmentation pod.) `cloudpipe-long-master-workflow-template.yaml` no longer declares `volumeClaimTemplates`. The "Decision" section above (the FastSurfer/registration EFS exception) is now historical only — no template in this pipeline mounts a PVC. The EFS filesystem, CSI driver, and StorageClass have been removed from the cluster and Terraform entirely.
