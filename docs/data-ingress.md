...
```
ingress-mode == globus:   start-globus-instance → globus-transfer → globus-s3-sync (skipped in S3-gateway mode)
ingress-mode != globus:   verify-staged-input
                                                    ↓
          inventory  ← depends: (sync Succeeded || Skipped) && (verify Succeeded || Skipped)
                                                    ↓
...
- **Every Globus task is guarded, and each accepts a `Skipped` upstream.** Argo
  ...
  failed `globus-transfer` also leaves `globus-s3-sync` Omitted, and inventory
  would run over a failed ingress.
...
| **Pre-staged S3** (`ingress-mode: presynced`) | You obtained the data another way (NDA download tool, an institutional copy, a collaborator's bucket) and put it in the bucket yourself | No | Supported — see below |
