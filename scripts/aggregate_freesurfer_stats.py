...
def run_batched(files: list[Path], cmd_factory, output: Path, batch_size: int) -> None:
...
def aggregate_aparc(
    files: list[Path], hemi: str, measure: str, output_dir: Path, delimiter: str, batch_size: int
) -> None:
    if not files:
